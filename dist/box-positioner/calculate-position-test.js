...
QUnit.test('test positions calculator', function (assert) {
    ...
    tests.forEach(t => {
        ...
        assert.deepEqual(calculate_point_1.default(calculationOptions), point, name);
        assert.deepEqual(calculate_position_1.default(calculationOptions), position, name);
    });
});
function args(x, y, frameOffset = 0, targetPadding = 0) {
    return {
...
function position(x, y, ...directions) {
    return {
        point: { x: x, y: y },
        direction: new direction_1.default(...directions)
    };
}
